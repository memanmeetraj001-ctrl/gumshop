...
// ==========================================
// 1. Super-Admin Specific Endpoints (Must be mounted BEFORE :slug)
// ==========================================
...
router.get('/super/all', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const stats = tenants.map((t) => {
      const prods = products.filter((p) => p.tenantId === t.id || (!p.tenantId && t.slug === 'demo'));
      const ords = orders.filter((o) => o.tenantId === t.id);
      const rev = ords.reduce((acc, o) => acc + (o.totalAmount || 0), 0);
      const owner = users.find((u) => u.tenantId === t.id || (t.slug === 'demo' && u.email === 'admin@gumshop.online'));

      return {
        ...t,
        ownerEmail: owner?.email || (t as any).ownerEmail || 'admin@gumshop.online',
        ownerName: owner?.name || t.storeName,
...
router.delete('/super/:id', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (id === 'tenant_demo' || id === 'demo') {
      res.status(400).json({ error: 'Cannot delete the primary demo store.' });
      return;
    }

    await db.saveState((s) => {
      s.tenants = (s.tenants || []).filter((t) => t.id !== id && t.slug !== id);
      s.products = (s.products || []).filter((p) => p.tenantId !== id);
      s.orders = (s.orders || []).filter((o) => o.tenantId !== id);
    });
...
// ==========================================
// 2. Public Storefront Slug Endpoints
// ==========================================
