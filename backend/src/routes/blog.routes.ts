...
router.post('/', authenticate, requireRole(['superadmin', 'editor']), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const newPost: BlogPost = {
      ...
      featuredImage: data.featuredImage || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=1200&q=80',
      author: data.author || (req.user ? req.user.name : 'GumShop Team'),
      authorAvatar: data.authorAvatar || '',
      categories: Array.isArray(data.categories) ? data.categories : ['Store Guides'],
